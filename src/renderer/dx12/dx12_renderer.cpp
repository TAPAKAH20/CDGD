...
void cg::renderer::dx12_renderer::load_pipeline()
{
	...
	// Swap chain
	DXGI_SWAP_CHAIN_DESC1 swap_cain_desc = {};
	swap_cain_desc.BufferCount = frame_number;
	swap_cain_desc.Width = settings->width;
	swap_cain_desc.Height = settings->height;
	swap_cain_desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	swap_cain_desc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	swap_cain_desc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	swap_cain_desc.SampleDesc.Count = 1;

	ComPtr<IDXGISwapChain1> temp_swap_chain;
	THROW_IF_FAILED(dxgi_factory->CreateSwapChainForHwnd(
		command_queue.Get(), cg::utils::window::get_hwnd(), &swap_cain_desc,
	...
	cbv_heap_desc.NumDescriptors = frame_number;
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	HRESULT result = D3DX12SerializeVersionedRootSignature(
		&rs_desc, rs_feature_data.HighestVersion, &signature, &error);
	...
	result = D3DCompileFromFile(
		shader_path.c_str(), nullptr, nullptr, "VSMain", "vs_5_0",
	...
	result = D3DCompileFromFile(
		shader_path.c_str(), nullptr, nullptr, "PSMain", "ps_5_0",
	...
	// Create and upload vertex buuffer
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtv_handle = {
		rtv_heap->GetCPUDescriptorHandleForHeapStart(),
		static_cast<int>(frame_index), rtv_descriptor_size
	};
